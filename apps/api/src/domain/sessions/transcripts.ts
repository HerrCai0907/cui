...
export function createRoundInputTranscript(
  session: ChatSession,
  round: number,
): string {
...
export function getRoundExecutionTrace(
  session: ChatSession,
  round: number,
): string {
  const responseIndex = findRoundResponseIndex(session, round);
  const searchEnd =
    responseIndex === -1 ? session.messages.length : responseIndex;
...
export function getRoundAssistantOutput(
  session: ChatSession,
  round: number,
): string {
  const responseIndex = findRoundResponseIndex(session, round);
